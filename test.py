...
ap.add_argument("-c", "--confidence", type=float, default=0.98,
...
''' linhnt31: 3/11/2021 - calculate the max temnperature and the respective frame'''
tempMax = []
indMax = 0
resFMax = 0 # the max temperature of tempMax
...
while True:
    ...
    ''' linhnt31: List of temperatures per frame '''
    tmpT = []
    ...
    for i in range(0, detections.shape[2]):
        ...
        vface_temp = round(np.max(data_array[y:y+h-50,x:w+x-20]), 2)
        tmpT.append(vface_temp)

        # dic[tuple(frame)].append(vface_temp)
