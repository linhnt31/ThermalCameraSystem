...
def draw_thermal_image(x, y, w, h):
    # khoi phuc nen trong
    fig.canvas.restore_region(ax_background)

    #doc anh nhiet tu camera
    mlx.getFrame(framebuf)
    # Lật ảnh và phóng ảnh nhiệt
    data_array = np.fliplr(np.reshape(framebuf, mlx_shape))  # reshape, flip data
...
while True:
	...
	for i in range(0, detections.shape[2]):
		...
		if count % 5 == 0: 
			...
			vface_temp = str(round(np.max(data_array[y:y+h,x:w+x]), 2))
			...
			cv2.putText(frame, vface_temp, (startX, y),
